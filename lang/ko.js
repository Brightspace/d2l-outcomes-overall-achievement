export default {
	anonymousUser: "익명 사용자",
	ascending: "오름차순",
	bigTrendAttemptsScreenReaderString: "{numAttempts, plural, =1 {회 시도 {attemptNames}} other {회 시도 {attemptNames} 및 {lastAttemptName}}}",
	cancelBtn: "취소",
	close: "닫기",
	commaSeparator: ",",
	descending: "내림차순",
	evidence: "근거",
	firstName: "이름",
	firstUseAlertSettingPageMessage: "설정에서 방법 세부 정보를 보거나 계산 방법을 변경합니다",
	firstUseAlertToastMessageNone: "숙련도 보기가 자동으로 계산되지 않습니다.",
	firstUseAlertToastMessage: "숙련도 보기는 \"{calculationMethod}\" 방법을 사용하여 자동으로 계산됩니다",
	goToNextPage: "표의 다음 페이지로 이동하십시오.",
	goToPreviousPage: "표의 이전 페이지로 이동하십시오.",
	goToUserAchievementSummaryPage: "스페이스 바나 엔터 키를 눌러 {name}님의 성취 요약 페이지를 엽니다.",
	headingDate: "날짜",
	headingEvidence: "근거 이름",
	headingLoa: "성취 수준",
	headingTotalAssessments: "종합 평가: {num}",
	indexedAttempt: "{Num}번 시도",
	labelOverallAchievement: "전체 성취도",
	lastName: "성",
	learnerHasNoData: "{Username}님의 전체 성취도 데이터가 없습니다.",
	learnerSortButtonDescription: "사용자 {newSortKey}{newSortDirection}으(로) 테이블 행을 정렬하려면 선택하십시오. 현재 정렬 순서는 {currentSortKey}{currentSortDirection}입니다.",
	levelNamePercentLabel: "{name}: {percentage}.",
	loadingOverallAchievement: "전체 성취도 로드.",
	manualOverride: "수동 재설정",
	masteryViewTableDescription: "숙련도 보기표: 이 표에는 각 학습자의 모든 강의 결과 전체 성취도가 표시되어 있습니다. 정렬된 활동이 있는 결과는 학급 분류 통계가 있는 열 머리글로 표시됩니다.",
	masteryViewTableEmptyError: "전체 성취도를 로드할 수 없습니다. 페이지를 새로 고치거나 오류가 계속 발생하는 경우 관리자에게 문의하십시오.",
	masteryViewUserOutcomeScreenReaderText: "{assessmentInfo}. 스페이스 바나 엔터 키를 눌러 평가 페이지를 엽니다.",
	miniTrendScreenReaderText: "평가됨 {numAssessed} {numAssessed, plural, =1 {회} other {회}}: {levelNames}",
	name: "이름",
	newPageSelectLiveText: "선택한 {totalPages}의 {pageNum} 페이지.",
	newPageSizeLiveText: "페이지당 {pageSize} 표시.",
	noAlignedOutcomes: "숙련도 보기를 사용하려면 먼저 {outcome, select, competencies {역량} expectations {기대} objectives {목표} outcomes {결과} standards {표준} other {표준}}}을(를) 활동 편집 페이지의 강의 활동에 맞게 정렬하십시오.",
	noEnrolledLearners: "강의에 등록한 학습자 없음",
	noScaleInstructor: "이 {outcome, select, competencies {역량} expectations {기대} objectives {목표} outcomes {결과} standards {표준} other {표준}}을(를) 성취하기 위해 학습자 진행 상황을 보려면, 성취 척도를 생성하고 선택합니다.",
	noScaleStudent: "{outcome, select, competencies {역량} expectations {기대} objectives {목표} outcomes {결과} standards {표준} other {표준}}}에 대한 성취를 측정하기 위해 어떠한 척도도 설정되지 않았습니다.",
	notAssessed: "평가되지 않음",
	notEvaluated: "평가되지 않음",
	notPublished: "게시되지 않음",
	outcomeInfo: "{name}. {description}",
	outOfDate: "유효 기간 지남",
	page: "페이지:",
	pageSelectOptionText: "{currentPage}/{pageCount}",
	pageSizeSelectOptionText: "페이지당 {pageSize}",
	percentLabel: "{percentage}%",
	periodSeparator: ".",
	published: "게시됨",
	releaseAllBtn: "모두 게시",
	releaseAllTxt: "강의 내 모든 학습자의 전체 성취도를 게시하시겠습니까?",
	retractAllBtn: "모두 철회",
	retractAllTxt: "강의 내 모든 학습자의 전체 성취도를 철회하시겠습니까?",
	searchResults: "{numResults} 검색 {numResults, plural, =1 {개 결과} other {개 결과}}",
	searchUsersLabel: "사용자 검색",
	searchUsersPlaceholder: "사용자 검색...",
	selectLearnersPerPage: "표의 각 페이지를 표시할 행 수를 선택하십시오.",
	selectTablePage: "표에서 특정 페이지를 표시하십시오.",
	toastReleased: "모든 학습자를 위해 게시된 전체 성취도",
	toastRetracted: "모든 학습자를 위해 철회된 전체 성취도",
	tooltipUserOutcomeAssessments: "{numTotal}개 활동 중 {numAssessed}개가 평가됩니다.",
	trend: "추세",
	untitled: "제목 없음",
	viewCourseIntentList: "{outcome, select, competencies {역량} expectations {기대} objectives {목표} outcomes {결과} standards {표준} other {표준}}} 도구에서 강의 인텐트 목록 보기"
};
